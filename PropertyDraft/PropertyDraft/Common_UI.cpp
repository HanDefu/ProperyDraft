...
std::vector<NXOpen::TaggedObject *> UI_GetSelectObjects(NXOpen::BlockStyler::UIBlock* block)
{
	...
	NXOpen::BlockStyler::BodyCollector* bodySelect = dynamic_cast<NXOpen::BlockStyler::BodyCollector*>(block);
	if(bodySelect != NULL)
	{
		//bodies
		pAttr = bodySelect->GetProperties();
		objs = pAttr->GetTaggedObjectVector("SelectedObjects");
	}

	NXOpen::BlockStyler::FaceCollector* faceSelect = dynamic_cast<NXOpen::BlockStyler::FaceCollector*>(block);
	if(faceSelect != NULL)
	{
		//bodies
		pAttr = faceSelect->GetProperties();
		objs = pAttr->GetTaggedObjectVector("SelectedObjects");
	}

	NXOpen::BlockStyler::CurveCollector* curveSelect = dynamic_cast<NXOpen::BlockStyler::CurveCollector*>(block);
	if(curveSelect != NULL)
	{
		//bodies
		pAttr = curveSelect->GetProperties();
		objs = pAttr->GetTaggedObjectVector("SelectedObjects");
	}
