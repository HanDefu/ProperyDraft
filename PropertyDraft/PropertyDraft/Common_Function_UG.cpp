...
int CF_InstanceGeometry2(tag_t body, tag_t plane, bool associate, tag_t &outbody)
{
	...
	int size = bodies2.size();
	outbody = bodies2[size-1];
...
int RY_CreateBoxWithCSYS_ForBody(tag_t solidBody, tag_t csys, NXOpen::Point3d pnt0, tag_t &boxFeat)
{
   ...
   // selectNXObjectList1 = toolingBoxBuilder1->FacetBodies();
    ...
    /*matrix1.Xx = -0.102603569774545;
    matrix1.Xy = 0.905275086254247;
    matrix1.Xz = 0.412249348910203;
    matrix1.Yx = -0.861029348131059;
    matrix1.Yy = 0.126697876636552;
    matrix1.Yz = -0.492520161732281;
    matrix1.Zx = -0.49809734904587;
    matrix1.Zy = -0.405493114939274;
    matrix1.Zz = 0.766468762971017;*/
...
int GetSplineAttachedToBody(const tag_t body, const vtag_t& allSplines, vtag_t& curSplines)
{
	...
	for( int idx = 0; idx < allSplines.size(); ++idx )
	{
		...
		tag_t proSpline = RY_Prototype(allSplines[idx]);
...
)
{
    ...
    printBuilder1->SetCopies(1);
    printBuilder1->SetThinWidth(1.0);
    printBuilder1->SetNormalWidth(1.0);
    printBuilder1->SetThickWidth(1.0);
	printBuilder1->SetWidth1ScaleFactor(2.0);
    printBuilder1->SetWidth2ScaleFactor(1.0);
    printBuilder1->SetWidth3ScaleFactor(1.0);
    printBuilder1->SetWidth4ScaleFactor(1.0);
    printBuilder1->SetWidth5ScaleFactor(2.0);//royal实线用的是0.7 对应的是这个 设置宽一点
    printBuilder1->SetWidth6ScaleFactor(1.0);
    printBuilder1->SetWidth7ScaleFactor(1.0);
    printBuilder1->SetWidth8ScaleFactor(1.0);
    printBuilder1->SetWidth9ScaleFactor(1.0);
    printBuilder1->SetOutput(NXOpen::PrintBuilder::OutputOptionWireframeBlackWhite);
    printBuilder1->SetRasterImages(true);
